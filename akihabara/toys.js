  ...
  * Top-view RPG specific libraries.
	...
	*/
	platformer:{
		...
		*
		*/
		getNextX:function(th) { 
			return th.x+th.accx/th.frames.walking.speed; 
		},
		...
		*/
		getNextY:function(th) { 
			var dy = help.limit(th.accy,-th.maxaccy,th.maxaccy);
			return th.y + dy; 
		},
		...
		*/
		verticalTileCollision:function(th,map,tilemap) {
			...
			var status = 'old y=' + th.y;
			if (map.tileIsSolidFloor(th,bottom) && th.accy >= 0) {
				th.accy=0;
				th.y=help.yPixelToTile(map,th.y+th.h)-th.h;
				th.touchedfloor=true;
			} else if (map.tileIsSolidCeil(th,top) && th.accy <= 0) {
				th.accy=0;
				th.y=help.yPixelToTile(map,th.y,1);
				th.touchedceil=true;
			}
			status += ' new y=' + th.y;
			gbox.setStatBar(status);
		},
		...
		*/
		horizontalTileCollision:function(th,map,tilemap,precision) {
			...
			while (t<th.h) {
				...
				if ((th.accx<0)&&map.tileIsSolid(th,left)) {
					th.accx=0;
					th.x=help.xPixelToTile(map,th.x-1,1);
					th.touchedleftwall=true;
				}
				if ((th.accx>0)&&map.tileIsSolid(th,right)) {
					th.accx=0;
