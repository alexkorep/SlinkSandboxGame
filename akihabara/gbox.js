...
var cyclelist={
	create:function(size) {
		return {
			...
			isBusy:function(){return this.isProcessing()||!this.isEnded()||gbox._isExternalLoadProcessing}, // There are elements in the queue/the last one pop returned an object that is being processed
...
var gbox={
	...
	// VARS
	_isExternalLoadProcessing: false,
	_autoid:0,
  ...
  */
	blitTile:function(tox,data) {
		...
		tox.translate((data.fliph?ts.tileimgw:0), (data.flipv?ts.tileimgh:0)); tox.scale((data.fliph?-1:1), (data.flipv?-1:1));
		this._safedrawimage(tox,img, ts.gapx+(ts.tileimgw*(data.tile%ts.tilerow)),ts.gapy+(ts.tileimgh*Math.floor(data.tile/ts.tilerow)),
			(data.w==null?ts.tileimgw:data.w),(data.h==null?ts.tileimgh:data.h),data.dx*(data.fliph?-1:1),data.dy*(data.flipv?-1:1),(data.w?data.w:ts.tileimgw),(data.h?data.h:ts.tileimgh));
		tox.restore();
	},
	
	blitTileWithMargins:function(tox,data,rightMargin,topMargin) {
		if (tox==null) return;
		var ts=this._tiles[data.tileset];
		var img=this.getImage(ts.image);
		this._implicitsargs(data);
		...
		this._safedrawimage(tox,img, 
			ts.gapx+(ts.tileimgw*(data.tile%ts.tilerow)), 						// source x
			ts.gapy+(ts.tileimgh*Math.floor(data.tile/ts.tilerow)) + topMargin, // source y
			(data.w==null?ts.tileimgw:data.w) - rightMargin, 					// source width
			(data.h==null?ts.tileimgh:data.h) - topMargin, 						// source height
			data.dx*(data.fliph?-1:1), 						// destination x
			data.dy*(data.flipv?-1:1) + topMargin,			// destination y
			(data.w?data.w:ts.tileimgw) - rightMargin, 		// destination width
			(data.h?data.h:ts.tileimgh) - topMargin			// destination height
		);
		tox.restore();
	},
